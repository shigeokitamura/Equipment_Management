$(window).on('load', () => {
  if ($('#borrowedBy').text() != "") {
    ...
    $('#borrowedBy').text('');
    $.ajax({
      url: '/equipment/users/userid_to_username?userid=' + user,
      type: 'GET',
      timeout: 10000
    }).done(data => {
      //console.log(data);
  ...
  if ($('#returnedBy').text() != "") {
    $.ajax({
        url: '/equipment/users/userid_to_username?userid=' + $('#returnedBy').text(),
        type: 'GET',
        timeout: 10000
    }).done(data => {
        //console.log(data);
...
function searchBarcodefromDB() {
  ...
  window.location.href = '/equipment/equipments/checkout?barcode=' + barcode;
}
...
function confirmCheckout(user, barcode) {
  if(window.confirm('この備品を借りますか？')) {
    ...
    form.method = 'POST';
    form.action = '/equipment/equipments/checkout';
...
function confirmReturn(user, barcode) {
  if(window.confirm('この備品を返却しますか？')) {
    ...
    form.method = 'POST';
    form.action = '/equipment/equipments/return';
